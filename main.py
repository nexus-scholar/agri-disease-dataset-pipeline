...
def train_fixmatch(model, source_loader, labeled_target_loader, unlabeled_target_loader, 
                   optimizer, device, epochs, healthy_idx):
    """Phase 2: FixMatch Training with Stability Checks"""
    ...
    for epoch in range(epochs):
        ...
        for _ in pbar:
            ...
            try: inputs_s, targets_s = next(iter_source)
            except StopIteration: iter_source = iter(source_loader); inputs_s, targets_s = next(iter_source)

            try: inputs_t, targets_t = next(iter_labeled)
            except StopIteration: iter_labeled = iter(labeled_target_loader); inputs_t, targets_t = next(iter_labeled)

            try: (inputs_u_w, inputs_u_s), _ = next(iter_unlabeled)
            except StopIteration: iter_unlabeled = iter(unlabeled_target_loader); (inputs_u_w, inputs_u_s), _ = next(iter_unlabeled)
            ...
            # Oracle Mapping: Map Noise (-1) to Healthy
            targets_t = torch.where(targets_t == -1, torch.tensor(healthy_idx).to(device), targets_t)
            ...
            valid_batches += 1
            pbar.set_postfix({'loss': total_loss / (valid_batches + 1e-6), 'mask': mask.mean().item()})
...
def main():
    ...
    print("\n=== Phase 2: FixMatch Training ===")
    # LOWER LR for stability (0.001 -> 0.0001)
    ...
    train_fixmatch(
        ...
        optimizer,
        device,
        epochs=20,              # Ensure this matches your desired length
        healthy_idx=healthy_idx # <--- ADD THIS LINE
    )
